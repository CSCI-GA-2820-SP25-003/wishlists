...
class TestProduct(TestCase):
    ...
    def test_filter_wishlist_products_by_price_range(self):
        """It should filter wishlist products by product price range based on query parameters."""
        # Create a wishlist and add several products with distinct prices
        wishlist = WishlistFactory()
        product1 = ProductFactory(wishlist=wishlist, price=9.99)
        product2 = ProductFactory(wishlist=wishlist, price=19.99)
        product3 = ProductFactory(wishlist=wishlist, price=29.99)
        product4 = ProductFactory(wishlist=wishlist, price=39.99)
        wishlist.products.extend([product1, product2, product3, product4])
        wishlist.create()  # Save the wishlist and its products to the database

        # Assume the endpoint URL is /wishlists/<wishlist_id>/products

        # Case 1: Filter by both min_price and max_price (should return product2 and product3)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?min_price=15&max_price=35")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertEqual(len(data["products"]), 2)
        prices = [p["price"] for p in data["products"]]
        self.assertIn(product2.price, prices)
        self.assertIn(product3.price, prices)

        # Case 2: Filter by only min_price (should return products with price >= 30: product3 and product4)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?min_price=30")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(all(p["price"] >= 30 for p in data["products"]))

        # Case 3: Filter by only max_price (should return products with price <= 15: product1)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?max_price=15")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(all(p["price"] <= 15 for p in data["products"]))

        # Case 4: No query parameters (should return all products)
        response = self.client.get(f"/wishlists/{wishlist.id}/products")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertEqual(len(data["products"]), 4)

        # Case 5: Invalid price values should return a 400 Bad Request
    ...
    def test_filter_wishlist_products_by_price_range(self):
        ...
        # Case 1: Filter by both min_price and max_price (expect product2 and product3)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?min_price=15&max_price=35")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertEqual(len(data["products"]), 2)
        ...
        # Case 2: Filter by only min_price (expect products with price >= 30: product3 and product4)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?min_price=30")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(all(p["price"] >= 30 for p in data["products"]))

        # Case 3: Filter by only max_price (expect products with price <= 15: product1)
        response = self.client.get(f"/wishlists/{wishlist.id}/products?max_price=15")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(all(p["price"] <= 15 for p in data["products"]))
